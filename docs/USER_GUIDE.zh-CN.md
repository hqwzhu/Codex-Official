...
- 第三方连接：使用 CCswitch/Freemodel、OpenRouter、硅基流动、私有网关等 OpenAI-compatible 服务。
...
点击 `使用官方 ChatGPT` 后，工具会把 Codex 切换回官方 `openai` provider 和 ChatGPT 账号登录模式。
...
已经打开的 Codex 窗口不会自动重新读取配置。请关闭并重新打开 Codex。
...
1. API Key 是否正确。
2. 模型名称是否是第三方网关支持的名称。
3. 接口类型是否正确，通常是 `responses` 或 `chat`。
